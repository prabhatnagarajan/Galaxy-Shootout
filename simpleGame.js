...
function Sprite(scene, imageFile, width, height){ 
  ...
  this.checkBounds = function(){
    ...
    offTop = false;
    offBottom = false;
    if(imageFile=="character2.png")
    {
      if(this.y>bottomBorder-52)
      {
	offBottom=true;
      }
      if(this.y<bottomBorder-299)
      {
	offTop=true;
      }
    }
    if(imageFile=="opponent2.png")
    {
      if(this.y<topBorder+52)
      {
	offTop=true;
      }
      if(this.y>bottomBorder-352)
      {
	offBottom=true;
      }
    }
	...
	this.x = leftBorder;
      ...
      if (offBottom){
	if (imageFile=="character2.png"||imageFile=="opponent2.png")
	{
	  if (imageFile=="character2.png")
	  {
	    this.y=bottomBorder-52;
	  }
	  if (imageFile=="opponent2.png")
	  {
	    this.y=bottomBorder-352;
	  }
	}
	else
	{
...
function Scene(){
    ...
    this.canvas.setAttribute("id","vangeffen");
    this.canvas.style.backgroundImage="url('nebula4.png')";
    //document.body.appendChild(this.canvas);
    //THE FOLLOWING CODE ENSURE THAT THE CANVAS IS INSERTED BEFORE THE FOOTER, SHOULD BE CHANGED  WHEN PLACED UPON ANOTHER WEBSITE
    var footer=document.getElementById("beforefooter");
